...
def build_consensus_graph(g_spatial, g_expr, alpha=0.9, k_c=15, 
                          weight_percentile=80, mutual_nn=False):
    """
    Build consensus graph with strict filtering.
    
    Args:
        g_spatial: spatial kNN graph (scipy sparse CSR matrix)
        g_expr: expression kNN graph (scipy sparse CSR matrix)
        alpha: weight for spatial graph (recommend 0.9-0.95 for stability)
        k_c: number of consensus neighbors to keep (recommend 10-15)
        weight_percentile: only keep edges with weight > this percentile (default 80)
        mutual_nn: if True, only keep mutual nearest neighbors
    
    Returns:
        neighbors: [N, k_c] array of neighbor indices
        weights: [N, k_c] array of edge weights
        weight_threshold: the actual threshold used
    """
    from scipy.sparse import csr_matrix
    import numpy as np
    
    # Ensure CSR format
    if not isinstance(g_spatial, csr_matrix):
        g_spatial = csr_matrix(g_spatial)
    if not isinstance(g_expr, csr_matrix):
        g_expr = csr_matrix(g_expr)
    
    n_nodes = g_spatial.shape[0]
    
    # Row-normalize both graphs for fair fusion
    from sklearn.preprocessing import normalize
    g_spatial_norm = normalize(g_spatial, norm='l1', axis=1)
    g_expr_norm = normalize(g_expr, norm='l1', axis=1)
    
    # Weighted fusion (still sparse)
    g_consensus = alpha * g_spatial_norm + (1 - alpha) * g_expr_norm
    g_consensus = csr_matrix(g_consensus)
    
    # Optional: mutual nearest neighbor filtering
    if mutual_nn:
        g_consensus_T = g_consensus.T.tocsr()
        # Keep only mutual edges
        g_mutual = g_consensus.minimum(g_consensus_T)
        g_consensus = g_mutual
    
    # Calculate weight threshold (top 20% by default)
    all_weights = g_consensus.data
    weight_threshold = np.percentile(all_weights, weight_percentile) if len(all_weights) > 0 else 0.0
    
    print(f"   Consensus graph weight threshold (top {100-weight_percentile}%): {weight_threshold:.4f}")
    
    # For each node, extract top-k HIGH-CONFIDENCE neighbors
    neighbors = np.zeros((n_nodes, k_c), dtype=np.int64)
    weights = np.zeros((n_nodes, k_c), dtype=np.float32)
    
    filtered_edge_count = 0
    total_candidate_count = 0
    
    for i in range(n_nodes):
        # Get non-zero neighbors in consensus graph
        row_start = g_consensus.indptr[i]
        row_end = g_consensus.indptr[i + 1]
        
        neighbor_indices = g_consensus.indices[row_start:row_end]
        neighbor_weights = g_consensus.data[row_start:row_end]
        
        total_candidate_count += len(neighbor_indices)
        
        # Filter by weight threshold (only high-confidence edges)
        high_conf_mask = neighbor_weights >= weight_threshold
        neighbor_indices = neighbor_indices[high_conf_mask]
        neighbor_weights = neighbor_weights[high_conf_mask]
        
        filtered_edge_count += high_conf_mask.sum()
        
        if len(neighbor_indices) == 0:
            # No high-confidence neighbors, use self-loop
            neighbors[i, 0] = i
            weights[i, 0] = 0.0
            continue
        
        # Select top-k from high-confidence candidates
        if len(neighbor_indices) > k_c:
            # argpartition: O(n) on average
            top_k_idx = np.argpartition(neighbor_weights, -k_c)[-k_c:]
            # Sort only the top-k
            top_k_idx = top_k_idx[np.argsort(neighbor_weights[top_k_idx])[::-1]]
        else:
            top_k_idx = np.argsort(neighbor_weights)[::-1]
        
        actual_k = min(len(neighbor_indices), k_c)
        neighbors[i, :actual_k] = neighbor_indices[top_k_idx[:actual_k]]
        weights[i, :actual_k] = neighbor_weights[top_k_idx[:actual_k]]
        
        # Pad with self-loop if not enough neighbors
        if actual_k < k_c:
            neighbors[i, actual_k:] = i
            weights[i, actual_k:] = 0.0
    
    retention_rate = filtered_edge_count / total_candidate_count if total_candidate_count > 0 else 0
    print(f"   Retained {filtered_edge_count}/{total_candidate_count} edges ({retention_rate:.1%}) after filtering")
    
    return neighbors, weights, weight_threshold
